...
FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    owner.SendMonsterMoveByPath(pointPath, 1, std::min<uint32>(pointPath.size(), 5), traveltime);
...
FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    if (Unit *fright = ObjectAccessor::GetUnit(owner, i_frightGUID))
    {
        ...
        if (dist_from_caster > 0.2f)
            ...
            angle_to_caster = frand(0, 2*M_PI);
    }
    else
    {
    ...
    if (dist_from_caster < MIN_QUIET_DISTANCE)
    {
        ...
        angle = angle_to_caster + frand(-M_PI/8, M_PI/8);
    }
    else if(dist_from_caster > MAX_QUIET_DISTANCE)
    {
        dist = frand(0.4f, 1.0f)*(MAX_QUIET_DISTANCE - MIN_QUIET_DISTANCE);
        angle = -angle_to_caster + frand(-M_PI/4, M_PI/4);
    }
...
template<class T>
void
FleeingMovementGenerator<T>::Interrupt(T &owner)
{
}

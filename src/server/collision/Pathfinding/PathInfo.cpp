...
////////////////// PathInfo //////////////////
PathInfo::PathInfo(const Unit* owner, float destX, float destY, float destZ,
bool useStraightPath, bool forceDest) : _polyLength(0), _type(PATHFIND_BLANK), _useStraightPath(useStraightPath),
_forceDestination(forceDest), _sourceUnit(owner), _navMesh(NULL), _navMeshQuery(NULL)
{
    ...
    if (_navMesh && _navMeshQuery && HaveTiles(endPoint) && !_sourceUnit->HasUnitState(UNIT_STAT_IGNORE_PATHFINDING))
    {
...
bool PathInfo::Update(float destX, float destY, float destZ, bool useStraightPath, bool forceDest)
{
    ...
    if (!_navMesh || !_navMeshQuery || !HaveTiles(newDest) ||
    ...
    float dist = _sourceUnit->GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS);
    if (inRange(oldDest, newDest, dist, dist) && _pathPoints.size() > 2)
    {
        // our target is not moving - we just coming closer
        // we are moving on precalculated path - enjoy the ride
 
        _pathPoints.crop(1, 0); 
        setNextPosition(_pathPoints[1]); 
...
dtPolyRef PathInfo::getPathPolyByPosition(const dtPolyRef *polyPath, uint32 polyPathSize, const float* point, float *distance)
{
...
void PathInfo::BuildPolyPath(const PathNode &startPos, const PathNode &endPos)
{
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (DT_SUCCESS != _navMeshQuery->closestPointOnPoly(suffixStartPoly, endPoint, suffixEndPoint))
        {
            ...
            sLog->outError("%u's Path Build failed: invalid polyRef in path", _sourceUnit->GetGUID());
        ...
        if (!suffixPolyLength || dtResult != DT_SUCCESS) 
        {
            ...
            sLog->outError("%u's Path Build failed: 0 length path", _sourceUnit->GetGUID());
        }
...
void PathInfo::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    for (uint32 i = 0; i < pointCount; ++i)
        _pathPoints.set(i, PathNode(pathPoints[i*VERTEX_SIZE+2], pathPoints[i*VERTEX_SIZE], pathPoints[i*VERTEX_SIZE+1]));
        ...
        (!(_type & PATHFIND_NORMAL) || !inRange(getEndPosition(), getActualEndPosition(), 1.0f, 1.0f)))
    {
        ...
        if (dist3DSqr(getActualEndPosition(), getEndPosition()) < 0.3f * dist3DSqr(getStartPosition(), getEndPosition()))
        {
            ...
            _pathPoints.set(_pathPoints.size()-1, getEndPosition());
        }
        else
        {
            setActualEndPosition(getEndPosition());
...
void PathInfo::BuildShortcut()
{
    ...
    _pathPoints.set(0, getStartPosition());
    _pathPoints.set(1, getActualEndPosition());
...
uint32 PathInfo::fixupCorridor(dtPolyRef* path, uint32 npath, uint32 maxPath, const dtPolyRef* visited, uint32 nvisited)
{
...
bool PathInfo::getSteerTarget(const float* startPos, const float* endPos, float minTargetDist, const dtPolyRef* path, uint32 pathSize, float* steerPos, unsigned char& steerPosFlag, dtPolyRef& steerPosRef)
 {
    // Find steer target.
...
const dtPolyRef* polyPath, uint32 polyPathSize, float* smoothPath, int* smoothPathSize,
